...
test "string indexing and utf8 encode/decode" {
  ...
  // WARNING: Important: Variables won't work with direct indexing
  let eq_char : Char = '='
  // s[0] == eq_char // BAD: Won't compile - eq_char is not a literal, lhs is UInt while rhs is Char
...
test "string interpolation basics" {
  ...
  // BAD: Wrong - quotes inside interpolation not allowed:
  // println("  - Checking if 'cache' section exists: \{config["cache"]}")

  // OK: Correct - extract to variable first:
