...
- Most project-scoped `moon` commands accept `--source-dir <dir>` so you can target a specific module directory; `moon update` is the exception. Use this when projects are nested, e.g. `moon check --source-dir nested/path`.
